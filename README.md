...
For non-trivial AAD work, the owner keeps one canonical task package with `<task-package>/plan.md` as the file-backed route ledger. The owner alone updates that ledger. Each child receives a distinct report/progress file under the task package and may append only there; integration reads those canonical files before relying on inline harness output. The default task package lives under `.pi/aad/tasks/`; `.pi/` is ignored durable AAD state and is never committed. Pi-subagents 0.34 still generates its own `.pi-subagents/` debug artifacts upstream; it remains ignored only as a runtime-compatibility path, while canonical AAD evidence lives in the task package. If report validation is unstable, raw evidence and validation diagnostics are retained and classified as `report-invalid` rather than discarded as an opaque task failure. This is intentionally not a versioned report-schema system.
