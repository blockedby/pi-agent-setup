...
For non-trivial routed work, identify the canonical owner ledger plus this report/progress path in `Context`: `<task-package>/plan.md` when committed task docs are required, otherwise `.pi-subagents/routes/<task-id>/<route-id>.md` with child files under `.pi-subagents/routes/<task-id>/`. The owner writes the ledger; this agent writes only its assigned report/progress file and may append dated status/comments there. `.pi-subagents/` is ignored generated runtime state and is not committed. Before synthesizing results, parents read these files rather than relying only on transient inline output or inferred harness paths.
