...
2. If no plan exists, choose a short, stable task slug from the task name and create `.pi/aad/tasks/YYYY-MM-DD-<task-slug>/`, unless the owner provides a repo-specific path.
3. Create `README.md` with:
   ...
   - PR URL, if available
   - report index
4. Create `plan.md` with the active plan coordinator, current task intake, repo orientation, reuse discovery, missing pieces, plan tasks, dependency graph, agent order, and execution ledger.
