...
3. Merge `R-*`, `F-*`, and `U-*` into the parent picture.
4. Resolve overlap between child results without reopening unnecessary rediscovery.
5. Decide whether the parent goal is now fully achieved, partially achieved, or still not achieved.
6. Produce a parent report with `aad-reporting`.
