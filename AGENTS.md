...
3. Check skill frontmatter when Pi reports `description is required` or a skill conflict:
   ...
   head -8 skills/<aad-common-or-general>/<skill-name>/SKILL.md
   ```
...
Keep `agents/`, `skills/aad/`, `skills/common/`, and `skills/general/` as the canonical sources. Common and general skills must remain free of AAD role, storage, and runtime coupling; the AAD setup composes all three roots. OpenCode compatibility belongs in `.opencode/plugins/pi-agent-setup.js` and deterministic tests, not in hand-maintained duplicate agent or skill trees.
