...
- `output` writes a requested single-agent output file; absolute paths remain absolute. Relative `/run` paths can use configured `singleRunOutputBaseDir`, but that setting does not relocate project debug artifacts; other multi-agent output paths resolve under their run directory. `outputMode: "file-only"` avoids returning saved output inline. Sessions are always enabled in 0.34; `sessionDir` selects the session-log location rather than enabling sessions.
...
Use task-package report/progress files as the durable AAD evidence record, not a guessed temporary artifact path or a run/session identifier. For non-trivial work, the owner ledger is `<task-package>/plan.md`, with distinct child report/progress files under the same task package. The default task package lives under `.pi/aad/tasks/`; `.pi/` is ignored canonical AAD state and is not committed. Pi-subagents 0.34 still creates `.pi-subagents/` debug artifacts upstream; that compatibility path is also ignored and is not the canonical ledger. Owners should record the async ID only as transient control metadata, read child files before integration, and preserve raw evidence plus validation diagnostics as `report-invalid` when validation is unstable.
